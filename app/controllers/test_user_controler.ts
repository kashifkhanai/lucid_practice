//this controller manages users, their profiles, and posts
...
export default class UsersController {
  ...
  // Get single user with profile & posts
  ...
  // Create profile for user (HasOne)
  public async createProfile(ctx: HttpContext) {
    try {
      const user = await getUserById(ctx.params.id)
      const payload = ctx.request.only(['bio'])
      const profile = await user.related('profile').create(payload)
      return SuccessService.send(ctx, 'PROFILE_CREATED', profile)
    } catch (error) {
      return ErrorService.handleError(ctx, error)
    }
  }

  // Create post for user (HasMany)
  public async createPost(ctx: HttpContext) {
    try {
      const user = await getUserById(ctx.params.id)
      const payload = ctx.request.only(['title', 'content'])
      const post = await user.related('posts').create(payload)
      return SuccessService.send(ctx, 'POST_CREATED', post)
    } catch (error) {
      return ErrorService.handleError(ctx, error)
    }
  }
